...
def searchGraph(problem, frontier):
  ...
  while not frontier.isEmpty():
    path = frontier.pop()
    last_successor = path[len(path) - 1]
    
    state = last_successor[0]

    if problem.isGoalState(state):
      directions = [x[1] for x in path[1:]]
      return directions
...
def depthFirstSearch(problem):
  ...
  "*** YOUR CODE HERE ***"
  frontier = util.Stack()
  return searchGraph(problem, frontier)
...
def aStarSearch(problem, heuristic=nullHeuristic):
  ...
  "*** YOUR CODE HERE ***"
  cost = lambda path: problem.getCostOfActions([x[1] for x in path[1:]]) +\
                      heuristic(path[len(path) - 1][0], problem)
  frontier = util.PriorityQueueWithFunction(cost)
  return searchGraph(problem, frontier)
